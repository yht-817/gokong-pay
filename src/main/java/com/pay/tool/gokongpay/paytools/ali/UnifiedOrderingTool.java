 ...
 * 根据业务需要调用当前的支付方法
 **/
public class UnifiedOrderingTool {
    private static final Logger log = LoggerFactory.getLogger(UnifiedOrderingTool.class);
     ...
     * @param coding    标题说明
     * @param orderno   订单编号
     * @param notifyurl 支付成功后异步通知
     * @return
     */
    public static Map<String, String> doUnifiedOrderApp(String money, String coding, String orderno, String notifyurl) {
     ...
     * 支付宝的网站支付接口
     */
    public static Map<String, String> doUnifiedOrderWeb(String money, String coding, String orderno, String notifyurl, String weburl) {
        Map<String, String> map = new HashMap<>();
        // 实例化支付对象
        AlipayClient alipayClient = AlipayConfig.getAlipayClient();
        ...
        alipayTradeWapPayRequest.setReturnUrl(weburl);
        try {
            //这里和普通的接口调用不同，使用的是sdkExecute
     ...
     * 支付宝进行PC网页二维码支付
     */
    public static String payPageCode(String money, String coding, String orderno, String notifyurl) {
        Map<String, String> map = new HashMap<>();
        // 实例化支付对象
        AlipayClient alipayClient = AlipayConfig.getAlipayClient();
        AlipayTradePagePayRequest alipayTradePagePayRequest = new AlipayTradePagePayRequest();
        AlipayTradePagePayModel AlipayTradePagePayModel = new AlipayTradePagePayModel();
        AlipayTradePagePayModel.setSubject(coding);      // 支付类型（标题）
        AlipayTradePagePayModel.setTotalAmount(money);   // 支付金额
        AlipayTradePagePayModel.setTimeoutExpress("90m");// 超时时间
        AlipayTradePagePayModel.setOutTradeNo(orderno);  // 订单号 （生成）
        // 业务参数
    ...
    /**
     * 支付宝支付成功后进行数据查询是否支付成功
     *
     * @param order_no 支付宝的订单编码或商户自己的支付编码
     * @return
     */
    public static String queryAliPay(String order_no) {
        AlipayClient alipayClient = AlipayConfig.getAlipayClient();
        ...
        try {
            ...
            log.error("订单查询返回的结果：" + response.toString());
     ...
     * 支付金额
     */
    public static String refundAliPay(String order_no, String amount) {
        AlipayClient alipayClient = AlipayConfig.getAlipayClient();
        ...
        try {
            ...
            log.info("退款的返回结果：" + tradeRefundResponse.toString());
            if (tradeRefundResponse.isSuccess()) {
                if (tradeRefundResponse.getCode().equals("10000")) {
                    ...
                    log.error("退款的接口再次重试失败：订单号：-----" + order_no + "金额：" + amount);
                    return "fail";
                } else {
                    return "fail";
                }
            } else {
                log.error("退款接口调用失败：订单号：-----" + order_no + "金额：" + amount);
                return "fail";
            }
        } catch (AlipayApiException e) {
            e.printStackTrace();
     ...
     */
    public static String closeAliPay(String order_no) {
        AlipayClient alipayClient = AlipayConfig.getAlipayClient();
        ...
        try {
            ...
            if (alipayTradeCloseResponse.isSuccess()) {
                if (alipayTradeCloseResponse.getCode().equals("10000")) {
                    ...
                    log.error("交易关闭订单再次重试失败：订单号：-----" + order_no);
                    return "fail";
                } else {
                    return "fail";
                }
            } else {
                log.error("退款查询接口调用失败：订单号：-----" + order_no);
                return "fail";
            }
        } catch (AlipayApiException e) {
            e.printStackTrace();
