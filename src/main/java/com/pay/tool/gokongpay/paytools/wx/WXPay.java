package com.pay.tool.gokongpay.paytools.wx;

import com.pay.tool.gokongpay.paytools.wx.WXPayConstants.SignType;
...
public class WXPay {
     ...
     * 该函数适用于商户适用于统一下单等接口，不适用于红包、代金券接口
     */
    public Map<String, String> fillRequestDatas(Map<String, String> reqData) throws Exception {
        reqData.put("appid", "wx3d011869f986cd83");
        reqData.put("mch_id", "1505189751");
        reqData.put("nonce_str", WXPayUtil.generateNonceStr());
        if (SignType.MD5.equals(this.signType)) {
            reqData.put("sign_type", WXPayConstants.MD5);
        } else if (SignType.HMACSHA256.equals(this.signType)) {
    ...
    public Map<String, String> againfillRequestDatas(Map<String, String> reqData) throws Exception {
        reqData.put("appid", "wx3d011869f986cd83");
        reqData.put("package", "Sign=WXPay");
        reqData.put("noncestr", WXPayUtil.generateNonceStr());
        reqData.put("partnerid", config.getMchID());
        reqData.put("timestamp", WXPayUtil.getTime());
        reqData.put("signType", String.valueOf(this.signType));
        reqData.put("sign", WXPayUtil.generateSignature(reqData, config.getKey(), this.signType));
        return reqData;
    }
    ...
    public String requestWithCert(String urlSuffix, Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        String msgUUID = reqData.get("nonce_str");
        String reqBody = WXPayUtil.mapToXml(reqData);
        String resp = this.wxPayRequest.requestWithCert(urlSuffix, msgUUID, reqBody, connectTimeoutMs, readTimeoutMs, true);
        return resp;
    }
     ...
     * 场景：公共号支付、扫码支付、APP支付
     */
    public Map<String, String> unifiedOrders(Map<String, String> reqData) throws Exception {
        return this.unifiedOrders(reqData, config.getHttpConnectTimeoutMs(), this.config.getHttpReadTimeoutMs());
    }
     ...
     * 场景：公共号支付、扫码支付、APP支付
     */
    public Map<String, String> unifiedOrders(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        String url;
        if (this.useSandbox) {
            url = WXPayConstants.SANDBOX_UNIFIEDORDER_URL_SUFFIX;
        } else {
        ...
        String respXml = this.requestWithoutCert(url, this.fillRequestDatas(reqData), connectTimeoutMs, readTimeoutMs);
        return this.processResponseXml(respXml);
    }
    ...
    public Map<String, String> againOrders(Map<String, String> reqData) throws Exception {
        return this.againfillRequestDatas(reqData);
    }
